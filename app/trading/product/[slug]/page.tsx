...
export default async function TradingProductDetail({ params }: Props) {
  const { slug } = await params
  const product = await getProductBySlug(slug)

  if (!product || !product.published || product.productType !== "TRADING") {
    return notFound()
  }
  ...
  return (
    <main className="bg-[#F7F9FC] min-h-screen">
      ...
      <ProductDetailContent
        ...
        breadcrumbBase={{ href: "/trading/product", label: "Trading & Distribution" }}
        detailBasePath="/trading/product"
      />
